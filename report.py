...
def generate_zz500_report():
    ...
    zz500_df = apply_strategy(zz500_df)

    filename = 'zz500_report_{}_{}'.format(
        datetime.now().strftime('%Y%B%d'), int(time.time()))
    save2file(filename, zz500_df)
...
def save2file(filename, df: pd.DataFrame):
